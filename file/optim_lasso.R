  ...
  D = NULL)
{
  ...
  UTCU<-function(coef,C){
    ...
    if (lambda.gam == 0){
      ...
      res<-hatQ[1]*nrow(ref_dat) + lambda.gam*sum(abs(coef[which(diag(D)!=0)]))
    }
    res
  }
  ...
  grad<-function(coef,C)
  {
    ...
    if(lambda.gam == 0){
      ...
      res_grad<-Dn/nrow(ref_dat)+lambda.gam*(diag(D)*sign(coef))
    }
    res_grad
  } 
  
  
  ## Define Hessian Matrix of Cost Function Q: for ridge why 
  ## there is no lambda for hessian 
