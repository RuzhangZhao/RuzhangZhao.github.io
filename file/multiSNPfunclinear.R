...
grad_U_wrt_beta_func<-function(
  ...
  beta){
  N_Pop<-nrow(UKBB_pop)
  U1_beta_gradient<-crossprod(UKBB_pop[,-1],UKBB_pop[,-1])*(-1)
...
gradient_utcu<-function(beta,C_current){
  ...
  # vector here 
  g0<-cbind(-R_mat,Z_mat)%*%C_current%*%u
...
Hessian_utcu<-function(beta,C_current){
  ...
  H<-crossprod(R_mat,cbind(-R_mat,Z_mat))%*%C_current%*%t(crossprod(R_mat,cbind(-R_mat,Z_mat)))
