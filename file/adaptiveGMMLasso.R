...
adaptiveGMMlasso3<-function(UKBB_pop,N_SNP,study_info){
  ...
  pseudo_Xy_list0<-pseudo_Xy(adiag(C_11_half*1e-8,C_22_half),UKBB_pop,var_SNP,var_GPC,N_SNP,theta_UKBB_GPC,study_info)
  initial_sf0<-N_Pop/sqrt(nrow(pseudo_Xy_list0$pseudo_X))
  pseudo_X0<-pseudo_Xy_list0$pseudo_X/initial_sf0
  pseudo_y0<-pseudo_Xy_list0$pseudo_y/initial_sf0
  
  ridge_fit<-glmnet(x= (pseudo_X0),y= (pseudo_y0),standardize=F,intercept=F,lambda = 1e-4,alpha = 0.01)
  #ridge_fit<-glmnet(x= scale(ref),y= scale(pheno_EUR$T2D,scale = F),standardize=F,intercept=F,lambda = lambda_list[50]/100/mean(w_adaptive),penalty.factor = w_adaptive)
  gamma_adaptivelasso<-1/2
  w_adaptive<-1/(abs(coef(ridge_fit)[-1]))^gamma_adaptivelasso
  w_adaptive[is.infinite(w_adaptive)]<-max(w_adaptive[!is.infinite(w_adaptive)])*10
