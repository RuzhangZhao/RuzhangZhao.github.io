  ...
  if(model == "logistic")
  {
    ...
    if (convergence){
      if(tune_lambda){
        ...
        lambda_tuned_convergence<-FALSE
        lambda_iter<-lambda_tune_initial
        while (total_iter_lam < maxit_lam &
            lambda_tune_tmp_eps > lambda_tune_eps){
          ...
          output_lam <- 
            useoptim(no_of_studies = no_of_studies, 
              ...
              return_C = FALSE,
              return_Hessian = TRUE,
              lambda.gam = lambda_iter,
              D = D)
          coef_iter_lam<-output_lam$estimated_coef
          H<-output_lam$Hessian
          total_iter_lam<-total_iter_lam+output_lam$no_of_iter
          update_lambda<-grad_lambda(lambda_iter,
          ...
          lambda_tune_tmp_eps<-abs(update_lambda)
          
          if (lambda_tune_tmp_eps <= lambda_tune_eps){
            lambda_tuned_convergence<-output_lam$convergence
          }
          REML<-c(output_lam$cost_val+log(det(H + lambda_iter*D))-sum(D)*log(lambda_iter) -(dim(D)[1]- sum(D))*log(2*pi)  )
          #print(paste0(lambda_iter,",",REML))
        }
        ...
        logistic_result<- list("estimated_coef" = coef_iter_lam,
          ...
          "no_of_iter_lambda_tune"=total_iter_lam,
          "lambda_tuned"= lambda_iter) 
